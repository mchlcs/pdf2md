...
from core.utils import _validar_existencia
...
def pdf_to_md(path: Path, ignorar_margens: float = 0.0) -> str:
    """
    ...
    try:
        partes = [
            _processar_pagina(doc, num, chunks, ignorar_margens)
...
def _processar_pagina(
    doc: fitz.Document, num_pagina: int, chunks: list[dict], ignorar_margens: float
) -> str:
